...
import { BrowserWrapper, ProxyConfig } from '@types';
...
export class PuppeteerService implements OnModuleDestroy {
  ...
  async onModuleInit() {
    for (const [idx, proxy] of PROXIES.entries()) {
      if (!proxy.host || !proxy.port || !proxy.username || !proxy.password) {
        this.logger.error(`[PuppeteerService] Proxy #${idx} is not fully configured`);
      }
    }
  }
  ...
  //   const getBrowserWithFreeSlot = () => pool.find((w) => w.reservedTabs < MAX_TABS);
  ...
  async createIsolatedPage(
    ...
    try {
      ...
      this.logger.error(
        `[DEBUG] Error in page.authenticate for proxy "${proxy.name}": ${e.message}`,
      );
    }
  ...
  private async createBrowser(locale: string, timeZone: string, proxy: ProxyConfig): Promise<Browser> {
  ...
  private getNextProxy(): ProxyConfig {
    ...
    this.logger.debug(
      `[PuppeteerService] Using proxy "${proxy.name}" (host=${proxy.host}:${proxy.port})`,
    );
